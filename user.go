...
func checkUser(username, password string) (bool, error) {
	// select * from users where username = 1111;
	...
	// assign the User type to variable utype
	var utype User
	// TypeOf  filter all search result of utype
...
func handlerLogin(w http.ResponseWriter, r *http.Request) {
	...
	if err := decoder.Decode(&user); err != nil {
		...
		fmt.Printf("Failed to read user data %v\n", err)
		return
	}

	exists, err := checkUser(user.Username, user.Password)
	if err != nil {
		http.Error(w, "Failed to check user from Elasticsearch", http.StatusInternalServerError)
		fmt.Printf("Failed to check user from Elasticsearch %v\n", err)
		return
	}
